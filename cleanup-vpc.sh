...
# Delete LBs.
lbs=$(aws elb describe-load-balancers | jq -r ".LoadBalancerDescriptions | .[] | select(.VPCId==\"$VPC_ID\") | select(.LoadBalancerName | startswith(\"milpa-\")) | .LoadBalancerName")
if [[ -n "$lbs" ]]; then
    echo "Removing LBs:"
    echo "$lbs"
    for lb in $lbs; do
...
sgs=$(aws ec2 describe-security-groups | jq -r ".SecurityGroups | .[] | select(.VpcId == \"$VPC_ID\")| select(.Tags) | select(.Tags[] | contains({\"Key\":\"MilpaClusterName\",\"Value\":\"$CLUSTER_NAME\"})) | .GroupId")
if [[ -n "$sgs" ]]; then
    echo "Removing SGs:"
    echo "$sgs"
    for sg in $sgs; do
